...
import Subscriber from '@cesium163-extends/subscriber';
...
import type { EventArgs, EventType } from '@cesium163-extends/subscriber';
...
export default class Drawer {
  ...
  private _typeClass!: Line | Polygon | Point | Rectangle | Circle | Triangle;
  ...
  set action(callback: ActionCallback | undefined) {
  ...
  private _initPainter(options: BasicGraphicesOptions): void {
    ...
    switch (this._type) {
      ...
      case 'RECTANGLE':
        ...
        break;
      case 'TRIANGLE':
        this._typeClass = new Triangle(this._painter, options);
        break;
    }
  ...
  start(
    ...
    const startId = this._subscriber.addExternal((move) => {
      ...
      if (this._typeClass.isComplete) this._endFunc(move, overrideFunc);
    ...
    const endId = this._subscriber.addExternal((move) => {
      ...
      this._endFunc(move, overrideFunc);
  ...
  private _endFunc(move: EventArgs, overrideFunc: OverrideEntityFunc): void {
    // 结束绘制，确定实体
    this._playOff(move);
  ...
  private _isSupport(type: string) {
    return ['POLYGON', 'POLYLINE', 'POINT', 'CIRCLE', 'RECTANGLE', 'TRIANGLE'].includes(type);
  }
